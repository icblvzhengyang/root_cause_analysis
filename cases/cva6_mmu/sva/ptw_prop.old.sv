...
asgpt__non_executable_instr: assert property (
    ptw.data_rvalid_q && ptw.is_instr_ptw_q && (!ptw.pte.x || !ptw.pte.a)
    |-> ptw.state_d == PROPAGATE_ERROR
);
...
asgpt__invalid_ppn_lvl1_lvl2: assert property (
    ((ptw.ptw_lvl_q == LVL1 && ptw.pte.ppn[17:0] != '0) || (ptw.ptw_lvl_q == LVL2 && ptw.pte.ppn[8:0] != '0))
    |-> ptw.state_d == PROPAGATE_ERROR
);
...
asgpt__store_on_clean_page_error: assert property (
    ptw.lsu_is_store_i && (!ptw.pte.w || !ptw.pte.d)
    |-> ptw.state_d == PROPAGATE_ERROR
);
...
asgpt__PTE_Lookup_ErrorCondition: assert property (
  disable iff (!rst_ni) 
    ptw.PTE_LOOKUP == $past(ptw.state_q) && (!ptw.pte.v || (!ptw.pte.r && ptw.pte.w)) |-> ptw.state_q == ptw.PROPAGATE_ERROR
);
...
asgpt__pte_lookup_error_conditions: assert property (
    (ptw.state_q == PTE_LOOKUP && (!ptw.pte.v || (!ptw.pte.r && ptw.pte.w))) |-> (ptw.state_d == PROPAGATE_ERROR)
);
...
asgpt__invalid_ppn_bits: assert property (
    (ptw.state_q == PTE_LOOKUP && (((ptw.ptw_lvl_q == LVL1) && (ptw.pte.ppn[17:0] != '0)) || ((ptw.ptw_lvl_q == LVL2) && (ptw.pte.ppn[8:0] != '0)))) |-> (ptw.state_d == PROPAGATE_ERROR)
);
...
asgpt__PTW_Error_Behavior: assert property ((ptw.pte.v == 1'b0 || (ptw.pte.r == 0 && ptw.pte.w)) |-> ptw_error_o);

// Asserting the global mapping behavior.
asgpt__Global_Mapping_Behavior: assert property (ptw.pte.g |-> ptw.global_mapping_n);

// Asserting proper TLB update for instructions.
asgpt__ITLB_Update_Behavior: assert property ((ptw.is_instr_ptw_q && ptw.pte.x && ptw.pte.a) |=> itlb_update_o.valid);

// Asserting proper TLB update for data.
asgpt__DTLB_Update_Behavior: assert property ((ptw.is_instr_ptw_q == 0'b0 && ptw.pte.a && (ptw.pte.r || (ptw.pte.x && mxr_i))) |=> dtlb_update_o.valid);

// Asserting that during a store operation in LSU, only writeable and dirty PTEs can be accessed.
asgpt__Store_PTE_Check: assert property ((lsu_is_store_i && (ptw.pte.w == 0 || ptw.pte.d == 0)) |=> (dtlb_update_o.valid == 0'b0 && ptw_error_o));

// Asserting the L1 PTE lookup behavior.
asgpt__L1_PTE_Lookup_Behavior: assert property ((ptw.ptw_lvl_q == LVL1 && ptw.pte.ppn[17:0] != '0) |=> ptw_error_o);

// Asserting the L2 PTE lookup behavior.
asgpt__L2_PTE_Lookup_Behavior: assert property ((ptw.ptw_lvl_q == LVL2 && ptw.pte.ppn[8:0] != '0) |=> ptw_error_o);
